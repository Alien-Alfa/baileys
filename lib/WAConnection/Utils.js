...
exports.extensionForMediaMessage = exports.decodeMediaMessageBuffer = exports.generateThumbnail = exports.getAudioDuration = exports.mediaMessageSHA256B64 = exports.ProxyAgent = exports.generateProfilePicture = exports.compressImage = exports.getMediaKeys = exports.decryptWA = exports.generateMessageID = exports.generateClientID = exports.generateMessageTag = exports.promiseTimeout = exports.delayCancellable = exports.delay = exports.debouncedTimeout = exports.unixTimestampSeconds = exports.randomBytes = exports.hkdf = exports.sha256 = exports.hmacSign = exports.aesEncrypWithIV = exports.aesEncrypt = exports.aesDecryptWithIV = exports.aesDecrypt = exports.shallowChanges = exports.newMessagesDB = exports.isGroupID = exports.whatsappID = exports.WA_MESSAGE_ID = exports.GET_MESSAGE_ID = exports.waMessageKey = exports.waChatKey = exports.toNumber = exports.Browsers = void 0;
...
const waChatKey = (pin) => ({
    key: (c) => (pin ? (c.pin ? '1' : '0') : '') +
        (c.archive === 'true' ? '0' : '1') +
        c.t.toString(16).padStart(8, '0') +
        c.jid,
    compare: (k1, k2) => k2.localeCompare(k1),
});
exports.waChatKey = waChatKey;
exports.waMessageKey = {
    key: (m) => (5000 + (m['epoch'] || 0)).toString(16).padStart(6, '0') +
        exports.toNumber(m.messageTimestamp).toString(16).padStart(8, '0'),
...
async function promiseTimeout(ms, promise) {
    ...
    const p = new Promise((resolve, reject) => {
        delay.then(() => reject(Constants_1.TimedOutError(stack))).catch((err) => reject(err));
...
const extractVideoThumb = async (path, destPath, time, size) => new Promise((resolve, reject) => {
    const cmd = `ffmpeg -ss ${time} -i ${path} -vframes 1 ${destPath}`;
...
async function getVideoDuration(path) {
    return new Promise((resolve, reject) => {
        fluent_ffmpeg_1.default.ffprobe(path, (err, metadata) => {
            ...
            if (err)
                reject(err);
            const duration = (_a = metadata === null || metadata === void 0 ? void 0 : metadata.format) === null || _a === void 0 ? void 0 : _a.duration;
...
async function generateThumbnail(buffer, mediaType, info) {
    if ('thumbnail' in info) {
        ...
        try {
            ...
            console_1.default.log('could not generate video thumb: ' + err);
        }
...
async function decodeMediaMessageBuffer(message, fetchRequest) {
    ...
    */
    ...
    if (fetched.status >= 400) {
        throw new Constants_1.BaileysError('Invalid code (' +
            fetched.status +
            ') returned. File has possibly been deleted from WA servers. Run `client.updateMediaMessage()` to refresh the url', { status: fetched.status });
    }
    ...
    for (let i = 0; i < allTypes.length; i++) {
        try {
            ...
            if (i > 0) {
                console_1.default.log(`decryption of ${type} media with HKDF key of ${allTypes[i]}`);
            }
            return decrypted;
        }
        catch (_b) {
            if (i === 0) {
                console_1.default.log(`decryption of ${type} media with original HKDF key failed`);
            }
        }
    }
